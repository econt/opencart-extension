<?php

// Heading
$_['heading_title'] = 'EcontPay';
$_['heading_title_2'] = 'EcontPay';

// Text
$_['text_home'] = 'Начало';
$_['text_extension'] = 'Модули';
$_['text_econt_payment'] = '<img src="view/image/payment/econt_payment.png" alt="EcontPay" title="EcontPay" style="width: 90px" />';
$_['text_success'] = 'Успешно редактирахте модула "EcontPay"';
$_['text_edit'] = 'Редактирай "EcontPay"';
$_['text_entry_geo_zone_all_zones'] = 'Всички зони';
$_['text_entry_status_enabled'] = 'Разрешено';
$_['text_entry_status_disabled'] = 'Забранено';
$_['text_entry_logo_dark'] = 'Тъмно';
$_['text_entry_logo_light'] = 'Светло';
$_['text_edit_order_statuses'] = 'Редакция на "Статуси на поръчки"';
$_['text_grid_gecd_column_header'] = 'EcontPay';
$_['text_grid_gecd_column_content_yes'] = 'Да';
$_['text_grid_gecd_column_content_no'] = 'Не';
// Entry
$_['entry_total'] = 'Общо сума';
$_['entry_order_status_payment_completed'] = 'Статус - Успешно плащане';
$_['entry_order_status_payment_processing'] = 'Статус - Очаква се плащане';
$_['entry_geo_zone'] = 'Гео зона';
$_['entry_status'] = 'Статус';
$_['entry_sort_order'] = 'Подретба';
$_['entry_title'] = 'Title';
$_['entry_logo'] = 'Logo';
$_['entry_description'] = 'Описание';

// Help
$_['help_entry_total'] = 'Общата сума на покупките трябва да надмине посочената преди този метод за заплащане да бъде активен.';

// Button
$_['button_save'] = 'Запази';
$_['button_cancel'] = 'Oтмени';

// Success
//$_['success_save'] = 'Success: XXX!';

// Error
//$_['error_permission'] = 'Warning: XXXX!';